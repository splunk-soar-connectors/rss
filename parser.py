...
class TextIOCParser:
    BASE_PATTERNS = [
        ...
        {"cef": "ip", "pattern": IPV6_REGEX, "name": "IP Artifact", "validator": _is_ip},
        {
            "cef": "requestURL",
            "pattern": URI_REGEX,
            "name": "URL Artifact",
        ...
        {"cef": "fileHash", "pattern": HASH_REGEX, "name": "Hash Artifact"},
        {
            ...
            "name": "Email Artifact",
            "subtypes": [
                {"cef": "domain", "name": "Domain Artifact", "callback": lambda x: x[x.rfind("@") + 1 :], "validator": lambda x: not _is_ip(x)}
            ],
        },
        {
            "cef": "email",
